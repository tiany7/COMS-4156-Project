...
class AuthServiceImpl final : public AuthService::Service {
    Status CheckValid(ServerContext* context, const CheckValidRequest* request, CheckValidResponse* response) override {
    ...
    Status IsLoggedIn(ServerContext* context, const CheckValidRequest* request, CheckValidResponse* response) override {
        std::cout << "CheckValid" << std::endl;
        Login login;
        auto username = request->user().username();
